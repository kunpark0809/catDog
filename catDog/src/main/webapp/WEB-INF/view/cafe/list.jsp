...
<div class="wide-container">
	 <div class="body-title">
		<span><i class="fas fa-utensils"></i> 카페/식당 </span>
		   ...
		   <tr height="40">
		      <td align="right">
		          <form name="searchForm" action="<%=cp%>/cafe/list" method="post" style="width: 100%;  border-bottom: 3px solid; border-bottom-width: 100%; padding-bottom: 10px; color: #d96262;">
		      ...
		      <c:if test="${sessionScope.member.userId=='admin' || sessionScope.member.userId=='admin2' || sessionScope.member.userId=='admin3'}">
		          <button type="button" class="bts" onclick="javascript:location.href='<%=cp%>/cafe/list';">새로고침</button>
		      </c:if>
		      </td>
		      <td align="left">
		      <c:if test="${sessionScope.member.userId=='admin' || sessionScope.member.userId=='admin2' || sessionScope.member.userId=='admin3'}">
		          <button type="button" class="bts" onclick="javascript:location.href='<%=cp%>/cafe/created';">등록하기</button>
		      </c:if>
		      </td>	      
		  	 </tr>
			</table>
						...
						<td width="20%" style="text-align: center;">
							<div class="place_image" onclick="javascript:location.href='${articleUrl}&recommendNum=${dto.recommendNum}'">
								<input type="hidden" value="${dto.recommendNum}">
									<img alt="" src="<%=cp%>/uploads/cafe/${dto.imageFileName}" style="padding-top: 15px; cursor:pointer; width: 380px; height: 380px; border-radius: 50px;"><br>
