...
function deleteFuneral(recommendNum) {
...
function updateFuneral(recommendNum) {
	<c:if test="${sessionScope.member.userId=='admin' || sessionScope.member.userId=='admin2' || sessionScope.member.userId=='admin3'}">
	var q = "recommendNum="+recommendNum+"&page=${page}";
    var url = "<%=cp%>/funeral/update?" + q;
...
<div class="page-section" id="command" style="text-align: center;">
		<div class="container">
			<div class="row">
				<div class="col-lg-12 text-center">
					<p class="section-heading text-uppercase" style="font-size: 45px; font-weight: bold;">장례식장</p>
				</div>
		...
		<table style="width: 98%; margin: 10px auto 0px; border-spacing: 0px; border-collapse: collapse; border-top: 3px solid;">
			...
			<br><br>
				<img alt="" src="<%=cp%>/uploads/funeral/${dto.imageFileName}">
			...
			<tr height="35" style="border-bottom: 1px solid #cccccc;">
			    ...
			    <br><br>
			       <i class="fas fa-chevron-up"></i>&nbsp;&nbsp;
			         <c:if test="${not empty preReadFuneral}">
			              <a style="color: #A66E4E" href="<%=cp%>/funeral/article?${query}&recommendNum=${preReadFuneral.recommendNum}">${preReadFuneral.placeName}</a>
			        </c:if>
			    </td>
			</tr>
			...
			<tr height="35" style="border-bottom: 1px solid #cccccc;">
			    <td colspan="2" align="left" style="padding-left: 5px;">
			       <i class="fas fa-chevron-down"></i>&nbsp;&nbsp;
			         <c:if test="${not empty nextReadFuneral}">
			              <a style="color: #A66E4E" href="<%=cp%>/funeral/article?${query}&recommendNum=${nextReadFuneral.recommendNum}">${nextReadFuneral.placeName}</a>
			        </c:if>
			    </td>
			</tr>
			</table>
			...
			<tr height="45">
			    <td align="center">
			    <br>
			        <button type="button" class="btn" onclick="javascript:location.href='<%=cp%>/funeral/list?${query}';">목록</button>
			    </td>
			 </table>  
			 ...
			 <tr>
			    <td align="right">
			       <c:if test="${sessionScope.member.userId=='admin' || sessionScope.member.userId=='admin2' || sessionScope.member.userId=='admin3'}">				    
			         <button type="button" class="btn" onclick="updateFuneral('${dto.recommendNum}');">수정</button>
			       </c:if>
			       <c:if test="${sessionScope.member.userId=='admin' || sessionScope.member.userId=='admin2' || sessionScope.member.userId=='admin3'}">				    
			         <button type="button" class="btn" onclick="deleteFuneral('${dto.recommendNum}');">삭제</button>
			       </c:if>
			    </td>
			</tr>
			</table>
