...
function searchList() {
	...
	<section class="page-section" id="command" style="text-align: center;">
		<div class="container">
		   ...
		   <tr height="40">
		      <td align="right">
		          <form name="searchForm" action="<%=cp%>/training/list" method="post" style="width: 100%;  border-bottom: 3px solid; border-bottom-width: 100%; padding-bottom: 10px;">
		      ...
		      <c:if test="${sessionScope.member.userId=='admin' || sessionScope.member.userId=='admin2' || sessionScope.member.userId=='admin3'}">
		          <button type="button" class="btn" onclick="javascript:location.href='<%=cp%>/training/list';">새로고침</button>
		      </c:if>
		      </td>
		      <td align="left">
		      <c:if test="${sessionScope.member.userId=='admin' || sessionScope.member.userId=='admin2' || sessionScope.member.userId=='admin3'}">
		          <button type="button" class="btn" onclick="javascript:location.href='<%=cp%>/training/created';">등록하기</button>
		      </c:if>
		      </td>	      
		  	 </tr>
			</table>
	...
	<div class="traininglist">	
			<c:forEach var="dto" items="${list}">
				<a onclick="javascript:location.href='${articleUrl}&recommendNum=${dto.recommendNum}'" style="color: #262626;">
					...
					<img alt="" src="<%=cp%>/uploads/training/${dto.imageFileName}" width="400" style="padding-top: 15px;"><br>
