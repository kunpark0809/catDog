...
function deleteTraining(recommendNum) {
...
function updateTraining(recommendNum) {
	<c:if test="${sessionScope.member.userId=='admin' || sessionScope.member.userId=='admin2' || sessionScope.member.userId=='admin3'}">
	var q = "recommendNum="+recommendNum+"&page=${page}";
    var url = "<%=cp%>/training/update?" + q;
...
$(function(){
	$(".btnSendRate").click(function(){
		...
		var url="<%=cp%>/training/insertRate";
...
$(function(){
	$("body").on("click", ".deleteRate", function(){
		...
		var url="<%=cp%>/training/deleteRate";
...
<div class="page-section" id="command" style="text-align: center;">
		<div class="container">
			<div class="row">
				<div class="col-lg-12 text-center">
					<p class="section-heading text-uppercase" style="font-size: 45px; font-weight: bold;">훈련/유치원</p>
				</div>
		...
		<table style="width: 98%; margin: 10px auto 0px; border-spacing: 0px; border-collapse: collapse; border-top: 3px solid;">
			...
			<br><br>
				<img alt="" src="<%=cp%>/uploads/training/${dto.imageFileName}">
			...
			<tr height="35" style="border-bottom: 1px solid #cccccc;">
			    ...
			    <br><br>
			       <i class="fas fa-chevron-up"></i>&nbsp;&nbsp;
			         <c:if test="${not empty preReadTraining}">
			              <a style="color: #A66E4E" href="<%=cp%>/training/article?${query}&recommendNum=${preReadTraining.recommendNum}">${preReadTraining.placeName}</a>
			        </c:if>
			    </td>
			</tr>
			...
			<tr height="35" style="border-bottom: 1px solid #cccccc;">
			    <td colspan="2" align="left" style="padding-left: 5px;">
			       <i class="fas fa-chevron-down"></i>&nbsp;&nbsp;
			         <c:if test="${not empty nextReadTraining}">
			              <a style="color: #A66E4E" href="<%=cp%>/training/article?${query}&recommendNum=${nextReadTraining.recommendNum}">${nextReadTraining.placeName}</a>
			        </c:if>
			    </td>
			</tr>
			</table>
			...
			<tr height="45">
			    <td align="center">
			    <br>
			        <button type="button" class="btn" onclick="javascript:location.href='<%=cp%>/training/list?${query}';">목록</button>
			    </td>
			 </table>  
			 ...
			 <tr>
			    <td align="right">
			       <c:if test="${sessionScope.member.userId=='admin' || sessionScope.member.userId=='admin2' || sessionScope.member.userId=='admin3'}">				    
			         <button type="button" class="btn" onclick="updateTraining('${dto.recommendNum}');">수정</button>
			       </c:if>
			       <c:if test="${sessionScope.member.userId=='admin' || sessionScope.member.userId=='admin2' || sessionScope.member.userId=='admin3'}">				    
			         <button type="button" class="btn" onclick="deleteTraining('${dto.recommendNum}');">삭제</button>
			       </c:if>
			    </td>
			</tr>
			</table>
